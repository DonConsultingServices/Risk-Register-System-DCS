...
class RiskController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Use form data if available, otherwise use calculated defaults
        $comprehensiveAssessment = \App\Models\ComprehensiveRiskAssessment::create([
            'risk_id' => $risk->id,
            'sr_risk_id' => $srRiskId,
            'sr_risk_name' => $request->sr_risk_name ?: 'Service Risk Assessment',
            'sr_impact' => $request->sr_impact ?: $this->getImpactFromPoints($srPoints),
            'sr_likelihood' => $request->sr_likelihood ?: $this->getLikelihoodFromPoints($srPoints),
            'sr_risk_rating' => $request->sr_risk_rating ?: $this->getRiskRatingFromPoints($srPoints),
            'sr_points' => $srPoints,
            'sr_mitigation' => $request->sr_mitigation ?: 'Standard service risk mitigation measures',
            'sr_owner' => $request->sr_owner ?: 'Risk Manager',
            'sr_status' => $request->sr_status ?: 'Open',
            
            'cr_risk_id' => $crRiskId,
            'cr_risk_name' => $request->cr_risk_name ?: 'Client Risk Assessment',
            'cr_impact' => $request->cr_impact ?: $this->getImpactFromPoints($crPoints),
            'cr_likelihood' => $request->cr_likelihood ?: $this->getLikelihoodFromPoints($crPoints),
            'cr_risk_rating' => $request->cr_risk_rating ?: $this->getRiskRatingFromPoints($crPoints),
            'cr_points' => $crPoints,
            'cr_mitigation' => $request->cr_mitigation ?: 'Standard client risk mitigation measures',
            'cr_owner' => $request->cr_owner ?: 'Risk Manager',
            'cr_status' => $request->cr_status ?: 'Open',
            
            'pr_risk_id' => $prRiskId,
            'pr_risk_name' => $request->pr_risk_name ?: 'Payment Risk Assessment',
            'pr_impact' => $request->pr_impact ?: $this->getImpactFromPoints($prPoints),
            'pr_likelihood' => $request->pr_likelihood ?: $this->getLikelihoodFromPoints($prPoints),
            'pr_risk_rating' => $request->pr_risk_rating ?: $this->getRiskRatingFromPoints($prPoints),
            'pr_points' => $prPoints,
            'pr_mitigation' => $request->pr_mitigation ?: 'Standard payment risk mitigation measures',
            'pr_owner' => $request->pr_owner ?: 'Risk Manager',
            'pr_status' => $request->pr_status ?: 'Open',
            
            'dr_risk_id' => $drRiskId,
            'dr_risk_name' => $request->dr_risk_name ?: 'Delivery Risk Assessment',
            'dr_impact' => $request->dr_impact ?: $this->getImpactFromPoints($drPoints),
            'dr_likelihood' => $request->dr_likelihood ?: $this->getLikelihoodFromPoints($drPoints),
            'dr_risk_rating' => $request->dr_risk_rating ?: $this->getRiskRatingFromPoints($drPoints),
            'dr_points' => $drPoints,
            'dr_mitigation' => $request->dr_mitigation ?: 'Standard delivery risk mitigation measures',
            'dr_owner' => $request->dr_owner ?: 'Risk Manager',
            'dr_status' => $request->dr_status ?: 'Open',
        ...
        // Ensure comprehensive assessment exists even if form data is missing
        $this->ensureComprehensiveAssessmentExists($client, $risk, $totalPoints, $overallRiskRating, $clientAcceptance, $ongoingMonitoring);
    ...
    /**
     * Ensure comprehensive assessment exists for client
     */
    private function ensureComprehensiveAssessmentExists($client, $risk, $totalPoints, $overallRiskRating, $clientAcceptance, $ongoingMonitoring)
    {
        // Check if comprehensive assessment already exists
        $existing = \App\Models\ComprehensiveRiskAssessment::where('risk_id', $risk->id)->first();
        
        if (!$existing) {
            // Calculate points for each category based on total points
            $crPoints = min(5, max(1, intval($totalPoints / 4)));
            $srPoints = min(5, max(1, intval($totalPoints / 4)));
            $prPoints = min(5, max(1, intval($totalPoints / 4)));
            $drPoints = $totalPoints - ($crPoints + $srPoints + $prPoints);
            if ($drPoints < 1) $drPoints = 1;
            
            // Create comprehensive assessment with calculated data
            \App\Models\ComprehensiveRiskAssessment::create([
                'risk_id' => $risk->id,
                ...
                'overall_risk_rating' => $overallRiskRating ?? 'Low',
                'client_acceptance' => $clientAcceptance ?? 'Accept client',
                'ongoing_monitoring' => $ongoingMonitoring ?? 'Annually',
    ...
    /**
     * Get likelihood level from points
     */
