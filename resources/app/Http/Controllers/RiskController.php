...
class RiskController extends Controller
{
    ...
    public function create()
    {
        ...
        // Get all risk examples for the dropdowns (SR, CR, PR, DR)
        $riskCategories = DB::table('risk_examples')
            ->join('business_risk_categories', 'risk_examples.business_risk_category_id', '=', 'business_risk_categories.id')
            ->select(
                'risk_examples.id',
                'risk_examples.risk_id',
                'risk_examples.title as risk_name',
                'risk_examples.detail as risk_detail',
                'risk_examples.impact',
                'risk_examples.likelihood',
                'risk_examples.risk_rating',
                'risk_examples.mitigation_strategies',
                'risk_examples.owner',
                'risk_examples.status',
                'business_risk_categories.code as risk_category',
                'business_risk_categories.name as category_name'
            )
            ->orderBy('business_risk_categories.code')
            ->orderBy('risk_examples.risk_id')
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        $validator->sometimes('company_nationality', 'required|in:Namibian,Foreign', function($input) {
            return $input->client_type === 'Juristic';
        });
        $validator->sometimes('foreign_registration', 'required|file|mimes:pdf,jpg,jpeg,png|max:5120', function($input) {
            return $input->client_type === 'Juristic' && $input->company_nationality === 'Foreign';
        });
        ...
        $client = \App\Models\Client::updateOrCreate(
            ...
            [
                ...
                'approved_at' => now(), // Set approval timestamp for review date calculation
                'approved_by' => auth()->id(), // Track who approved the assessment
        ...
        // Handle document uploads - Individual documents
        ...
        $registrationPath = $request->file('registration_document')?->store('kyc/company_registration', 'public');
        $foreignRegistrationPath = $request->file('foreign_registration')?->store('kyc/foreign_registration', 'public');
        $taxCertificatePath = $request->file('tax_certificate')?->store('kyc/tax_certificates', 'public');
        ...
        $risk = Risk::create([
            ...
            'foreign_registration_path' => $foreignRegistrationPath,
        ...
        $documentsToSave = [
            ...
            // Juristic-specific documents
            ['type' => 'registration_document', 'path' => $registrationPath],
            ['type' => 'foreign_registration', 'path' => $foreignRegistrationPath],
            ['type' => 'tax_certificate', 'path' => $taxCertificatePath],
        ];
    ...
    public function backupDatabase()
    {
        try {
            ...
            if ($returnVar === 0 && file_exists($filepath)) {
                ...
                // Fallback: Use Laravel's database export
                return $this->fallbackDatabaseBackup();
            }
        } catch (\Exception $e) {
    ...
    /**
     * Fallback database backup using PHP
     */
    private function fallbackDatabaseBackup()
    {
        try {
            $filename = 'database_backup_' . date('Y-m-d_H-i-s') . '.sql';
            $filepath = storage_path('app/backups/' . $filename);
            ...
            Log::info('Fallback database backup created', [
                'filename' => $filename,
                'user_id' => auth()->id(),
                'size' => filesize($filepath)
            ]);
            ...
            Log::error('Fallback database backup failed', [
                'error' => $e->getMessage(),
                'user_id' => auth()->id()
            ]);
    ...
    public function backupFull()
    {
        try {
            ...
            $zipFilename = 'full_system_backup_' . $timestamp . '.zip';
            ...
            // 2. Add storage files (uploaded documents)
            $storagePublicPath = storage_path('app/public');
            if (is_dir($storagePublicPath)) {
                $this->addFolderToZip($storagePublicPath, $zip, 'storage/');
            }
            
            // 3. Add public storage files
            $publicStoragePath = public_path('storage');
            if (is_dir($publicStoragePath)) {
                $this->addFolderToZip($publicStoragePath, $zip, 'public_storage/');
            }
            
            // 4. Add .env file (important for configuration)
            ...
            Log::info('Full system backup created', [
            ...
            return redirect()->route('risks.settings')
                ->with('error', 'Full system backup failed: ' . $e->getMessage());
        }
    }
