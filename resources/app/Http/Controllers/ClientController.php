...
class ClientController extends Controller
{
    ...
    public function modalDetails(Client $client)
    {
        ...
        // Check if columns exist first before querying
        $documentData = [];
        try {
            ...
            // Columns don't exist yet - database needs migrations
            \Log::error("Document columns missing in risks table: " . $e->getMessage());
        }

        // Merge KYC data into client object
        if (!empty($kycData)) {
            $kyc = $kycData[0];
            $client->client_type = $kyc->client_type;
            $client->gender = $kyc->gender;
            $client->nationality = $kyc->nationality;
            $client->is_minor = $kyc->is_minor;
            $client->id_number = $kyc->id_number;
            $client->passport_number = $kyc->passport_number;
            $client->registration_number = $kyc->registration_number;
        ...
        if (!empty($documentData) && isset($documentData[0])) {
            $docs = $documentData[0];
            $client->id_document_path = $docs->id_document_path ?? null;
            $client->birth_certificate_path = $docs->birth_certificate_path ?? null;
            $client->passport_document_path = $docs->passport_document_path ?? null;
            $client->proof_of_residence_path = $docs->proof_of_residence_path ?? null;
            $client->kyc_form_path = $docs->kyc_form_path ?? null;
        } else {
            // Set defaults if no document data
            $client->id_document_path = null;
            $client->birth_certificate_path = null;
            $client->passport_document_path = null;
            $client->proof_of_residence_path = null;
            $client->kyc_form_path = null;
        }
