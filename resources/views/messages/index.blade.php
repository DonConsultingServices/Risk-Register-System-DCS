...
@section('title', 'Messages - DCS-Best')
...
<div class="messages-container">
    ...
    <div class="filter-tabs">
        ...
        <a href="{{ route('messages.index', ['filter' => 'read']) }}" 
           class="filter-tab {{ $filter === 'read' ? 'active' : '' }}">
            Read ({{ $counts['read'] }})
        </a>
        <a href="{{ route('messages.index', ['filter' => 'important']) }}" 
           class="filter-tab {{ $filter === 'important' ? 'active' : '' }}">
            Important ({{ $counts['important'] }})
        </a>
        <a href="{{ route('messages.index', ['filter' => 'urgent']) }}" 
    ...
    @if($messages->count() > 0)
        @foreach($messages as $message)
            <div class="message-card {{ $message->isUnread() ? 'unread' : 'read' }} {{ $message->is_important ? 'important' : '' }} {{ $message->priority === 'urgent' ? 'urgent' : '' }}"
                 onclick="window.location.href='{{ route('messages.show', $message) }}'">
    ...
    @else
        <div class="empty-state">
            ...
            <a href="{{ route('messages.create') }}" class="btn btn-dcs-primary">
